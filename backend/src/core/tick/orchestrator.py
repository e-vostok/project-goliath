...
class TickOrchestrator:
    ...
    async def run_tick(cls, session: AsyncSession) -> None:
        ...
        from core.db import get_session_maker
        log_session_maker = get_session_maker()
        ...
        try:
            ...
            async with log_session_maker() as log_session:
                from modules._00_core.models import TickLog, TickLogStatus
...
class TickOrchestrator:
    ...
    async def run_tick(cls, session: AsyncSession) -> None:
        ...
        # Create tick log entry using a SEPARATE session/connection
        # This ensures the log survives a rollback of the main tick transaction
        # We use session.bind which returns the AsyncEngine
        async_engine = session.bind
        from sqlalchemy.ext.asyncio import AsyncSession
        async with AsyncSession(async_engine) as log_session:
            # Get the next ID for tick_log (SQLite limitation workaround)
            from sqlalchemy import func
            result = await log_session.execute(
                select(func.max(TickLog.id))
            )
